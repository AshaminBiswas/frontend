...
import { PRODUCT_SUB_CATEGORIES } from "../../data/products";

export function ProductsDropdown({ onSelectProduct }: { onSelectProduct?: (prod: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {open && (
        ...
        <div className="md:absolute md:top-full md:left-0 z-50 md:min-w-[240px] md:pt-1 md:animate-in md:fade-in md:slide-in-from-top-3 md:duration-200">
          <ul className="bg-[#2a0e08] md:border md:border-[#EACEAA]/20 md:rounded-tr-2xl md:rounded-bl-2xl overflow-hidden md:shadow-2xl py-1">
            {PRODUCT_SUB_CATEGORIES.map((item, i) => (
              <li key={item.label}>
                <button
                  type="button"
                  onClick={() => {
                    if (onSelectProduct) onSelectProduct(item.label);
                    setOpen(false);
                  }}
                  className={`w-full text-left px-5 py-2.5 text-sm text-[#EACEAA]/85 hover:text-[#D39858] hover:bg-[#EACEAA]/10 transition-all duration-200 ${
                    i !== PRODUCT_SUB_CATEGORIES.length - 1 ? "border-b border-[#EACEAA]/8" : ""
                  }`}
                >
                  {item.label}
                </button>
