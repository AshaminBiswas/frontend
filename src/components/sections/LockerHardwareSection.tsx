import { LOCKER_SECTION } from "../../data/products";
import { ImageWithFallback } from "../figma/ImageWithFallback";
import { useInView } from "../../hooks/useInView";

interface LockerHardwareSectionProps {
  onSelectCategory?: (title: string) => void;
}

export function LockerHardwareSection({ onSelectCategory }: LockerHardwareSectionProps) {
  ...
  return (
    <section className="py-4 sm:py-6 md:py-10 px-3 sm:px-6 md:px-8 lg:px-16 overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 overflow-hidden">
        {LOCKER_SECTION.map((item, i) => (
          <div key={item.id} className="w-full overflow-hidden">
            <LockerCard key={item.id} item={item} index={i} onSelectCategory={onSelectCategory} />
          </div>
        ))}
      </div>
    </section>
...
function LockerCard({
  ...
  return (
    ...
    >
      <div
        className="relative overflow-hidden rounded-tr-2xl rounded-bl-2xl sm:rounded-tr-3xl sm:rounded-bl-3xl group cursor-pointer shadow-2xs hover:shadow-md h-[150px] sm:h-[220px] md:h-[280px] lg:h-[300px] w-full bg-[#34150F]"
      ...
      >
        ...
        <div className="relative z-20 h-full flex flex-col justify-end p-3.5 sm:p-6 md:p-8">
          ...
          <button
            type="button"
            className="self-start bg-[#EACEAA] text-[#34150F] text-[10px] sm:text-xs font-bold px-3 py-1 sm:px-4 sm:py-2 rounded-tr-md rounded-bl-md sm:rounded-tr-xl sm:rounded-bl-xl hover:bg-[#D39858] transition-all duration-300 shadow-2xs active:scale-95"
          >
            Shop Now
          </button>
        </div>
