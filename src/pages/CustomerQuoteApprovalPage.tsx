...
export function CustomerQuoteApprovalPage() {
  ...
  return (
    <div className="min-h-screen bg-[#EACEAA]/20 py-4 sm:py-8 px-2.5 sm:px-6 md:px-8 lg:px-16 print:bg-white print:p-0" style={{ fontFamily: "'Nunito', sans-serif" }}>
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
        {/* Action Header - Hidden during print */}
        <div className="flex flex-wrap items-center justify-between gap-2.5 sm:gap-4 print:hidden">
          <button
            type="button"
            onClick={() => navigate(-1)}
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="bg-white border border-[#34150F]/15 hover:border-[#34150F] text-[#34150F] font-bold text-[11px] sm:text-xs px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl transition-all shadow-2xs flex items-center gap-1.5"
            >
              <Printer size={13} />
              <span>Print</span>
            </button>
        ...
        <div
          ref={printAreaRef}
          className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-10 border border-[#34150F]/10 shadow-md space-y-5 sm:space-y-8 print:border-none print:shadow-none print:p-4 print:rounded-none"
        >
          {/* Header & Company Brand */}
          <div className="flex flex-wrap items-start justify-between gap-3 sm:gap-6 border-b border-[#34150F]/15 pb-4 sm:pb-6">
            <div className="flex items-start gap-3 sm:gap-5">
              <img src="/logo.png" alt="PRC Logo" className="w-12 h-12 sm:w-20 sm:h-20 object-contain shrink-0" />
              <div>
                <span className="text-[9px] sm:text-[10px] font-extrabold uppercase tracking-widest text-[#D39858] bg-[#D39858]/10 px-2 py-0.5 rounded border border-[#D39858]/20">
                  Commercial Quotation
                </span>
                <h1 className="text-xl sm:text-3xl font-black text-[#34150F] mt-0.5 sm:mt-1" style={{ fontFamily: "'Gilda Display', serif" }}>
                  PRC Hardware
                </h1>
                <p className="text-[10.5px] sm:text-xs text-[#85431E] mt-0.5 font-medium leading-tight">
                  H -3, J.R. COMPLEX GATE NO 4, MELA RAM FARM, MANDOLI, DELHI 110093, INDIA
                </p>
                <p className="text-[9.5px] sm:text-[11px] text-[#85431E]/70 mt-0.5 font-mono">
                  GSTIN: 27AABCP1234F1Z9 • support@pacifichardware.com
                </p>
              </div>
            </div>

            {/* Document Metadata */}
            <div className="bg-[#EACEAA]/20 p-2.5 sm:p-4 rounded-xl sm:rounded-2xl border border-[#34150F]/10 text-left sm:text-right space-y-1 w-full sm:w-auto sm:min-w-[180px]">
              <div>
                <span className="text-[9px] text-[#85431E] uppercase font-bold block">Quotation Ref No</span>
                <span className="font-mono font-black text-xs text-[#34150F]">{quote.referenceNo}</span>
              </div>
              <div className="pt-0.5">
                <span className="text-[9px] text-[#85431E] uppercase font-bold block">Date of Issue</span>
              ...
              <div className="pt-0.5">
                <span className="text-[9px] text-[#85431E] uppercase font-bold block">Status</span>
                <span className={`text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded-full uppercase inline-block ${
                  quote.status === "APPROVED"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 bg-[#EACEAA]/10 p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border border-[#34150F]/10 text-xs">
            <div className="space-y-0.5 sm:space-y-1">
              <span className="text-[9px] sm:text-[10px] uppercase font-bold text-[#85431E] tracking-wider block">Billed To (Client)</span>
              <p className="font-extrabold text-xs sm:text-sm text-[#34150F]">{quote.companyName}</p>
              <p className="text-[#34150F] font-semibold text-[11px] sm:text-xs">Attn: {quote.firstName} {quote.lastName}</p>
              <p className="font-mono text-[#85431E] text-[10px] sm:text-xs">GSTIN: {quote.gstNo}</p>
              <p className="text-[#85431E] text-[10.5px] sm:text-xs">{quote.email} • {quote.phone}</p>
            </div>

            <div className="space-y-0.5 sm:space-y-1 sm:text-right pt-2 sm:pt-0 border-t sm:border-t-0 border-[#34150F]/10">
              <span className="text-[9px] sm:text-[10px] uppercase font-bold text-[#85431E] tracking-wider block">Project Scope</span>
              <p className="font-extrabold text-xs sm:text-sm text-[#34150F]">{quote.projectName}</p>
              {quote.notes && (
                <p className="text-[#85431E] italic text-[10px] sm:text-[11px] leading-relaxed pt-0.5">
                  "{quote.notes}"
                </p>
              )}
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="md:hidden space-y-1.5">
              {quote.items.map((item, idx) => (
                <div key={item.id} className="bg-[#EACEAA]/15 p-2.5 rounded-lg border border-[#34150F]/10 space-y-1">
                  <div className="flex items-start justify-between gap-1.5">
                    <div>
                      <p className="text-[11.5px] font-bold text-[#34150F] leading-tight">{item.productNameSnapshot}</p>
                      {item.product?.sku && (
                        <p className="text-[9px] font-mono text-[#85431E]/70">SKU: {item.product.sku}</p>
                      )}
                    </div>
                    <span className="text-[9px] font-bold text-[#85431E] bg-white px-1.5 py-0.2 rounded border border-[#34150F]/10">
                      #{idx + 1}
                    </span>
                  </div>

                  <div className="flex items-center justify-between text-xs pt-1 border-t border-[#34150F]/6">
                    <span className="text-[10.5px] text-[#85431E]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 pt-1">
            ...
            <div className="p-3.5 sm:p-4 bg-slate-900 text-white rounded-xl sm:rounded-2xl border border-slate-800 space-y-2.5 sm:space-y-3">
              ...
              <div className="flex items-center gap-3">
                {quote.qrCodeData ? (
                  <img
                    ...
                    className="w-18 h-18 sm:w-24 sm:h-24 bg-white p-1 rounded-lg sm:rounded-xl shadow-md shrink-0"
                  />
                ) : (
                  <div className="w-18 h-18 sm:w-24 sm:h-24 bg-slate-800 rounded-lg sm:rounded-xl flex items-center justify-center text-slate-500 shrink-0">
                    <QrCode size={28} />
                  </div>
                )}

                <div className="space-y-0.5 text-[10px] sm:text-[11px] leading-tight">
                  <p className="text-slate-300">
                    <strong>Signed:</strong> {quote.signedBy || "PRC Authority"}
                  </p>
                  <p className="text-slate-400">
                    <strong>Time:</strong>{" "}
                    {quote.signedAt
                      ? new Date(quote.signedAt).toLocaleDateString("en-IN")
              ...
              {quote.digitalSignature && (
                <div className="pt-1.5 border-t border-slate-800 flex items-center justify-between text-[9px] font-mono text-slate-400">
                  <span className="truncate max-w-[140px] sm:max-w-[180px]">SHA256: {quote.digitalSignature}</span>
            ...
            <div className="p-3.5 sm:p-5 bg-[#EACEAA]/25 rounded-xl sm:rounded-2xl border border-[#34150F]/15 space-y-2 text-xs">
              ...
              <div className="flex items-center justify-between text-[11px] sm:text-xs">
                <span className="text-[#85431E] font-semibold">Shipping / Logistics</span>
              ...
              <div className="flex items-center justify-between text-xs sm:text-base font-extrabold text-[#34150F] pt-2 border-t border-[#34150F]/15">
                <span>Grand Total (INR)</span>
                <span className="font-mono text-sm sm:text-lg text-[#85431E]">₹{quote.grandTotal.toLocaleString("en-IN")}</span>
              </div>

              {/* Advance Payment Requirement Display */}
              <div className="pt-1.5 border-t border-[#34150F]/15 bg-[#EACEAA]/30 p-2.5 rounded-lg sm:rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-[#34150F] font-bold text-[11px] sm:text-xs block flex items-center gap-1">
                    <Percent size={11} className="text-[#D39858]" />
                    <span>Advance Terms</span>
                  </span>
                  <span className="text-[9px] text-[#85431E]">
                    {quote.advancePercentage !== null && quote.advancePercentage !== undefined
                      ? "Configured by Estimating Team"
                      : "Standard B2B Terms"}
                  </span>
                </div>
                <div className="text-right">
                  <span className="font-mono font-black text-[11px] sm:text-xs text-[#85431E] bg-white px-1.5 py-0.2 rounded border border-[#34150F]/15 inline-block">
                    {quote.advancePercentage !== null && quote.advancePercentage !== undefined
                      ? `${quote.advancePercentage}%`
                      : "30%"}
                  </span>
                  <span className="block font-mono font-bold text-[10px] sm:text-[11px] text-[#34150F] mt-0.5">
                    ₹{Math.round(
                      quote.grandTotal *
                        ((quote.advancePercentage !== null && quote.advancePercentage !== undefined
                          ? quote.advancePercentage
          ...
          <div className="text-[9.5px] sm:text-[10px] text-[#85431E]/70 space-y-0.5 pt-3 border-t border-[#34150F]/10 leading-relaxed">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-[#34150F]/10 shadow-md space-y-3.5 sm:space-y-5 print:hidden">
          <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-[#34150F]/10 pb-3">
            <h3 className="text-sm sm:text-base font-bold text-[#34150F] flex items-center gap-1.5">
              <Send size={15} className="text-[#D39858]" />
            ...
            {quote.status === "APPROVED" && (quote.canCustomerEdit || (!quote.customerEditCount && quote.customerResponse === "pending")) && (
              <button
                ...
                className="bg-[#EACEAA]/40 hover:bg-[#D39858] text-[#34150F] font-bold text-[10.5px] sm:text-xs px-3 py-1.5 rounded-lg sm:rounded-xl transition-all border border-[#34150F]/15 flex items-center gap-1 shadow-2xs"
              >
                <Edit3 size={12} className="text-[#85431E]" />
                <span>Change Terms / Advance %</span>
              </button>
            )}
          </div>
          ...
          {isResponded ? (
            <div className="p-3.5 sm:p-5 bg-[#EACEAA]/20 rounded-xl sm:rounded-2xl border border-[#34150F]/10 space-y-2">
              <div className="flex items-center gap-2 font-bold text-xs">
                {quote.customerResponse === "accepted" ? (
                  <span className="text-emerald-700 flex items-center gap-1 font-extrabold text-xs sm:text-sm">
                    <CheckCircle2 size={14} /> Quotation Accepted
                  </span>
                ) : (
                  <span className="text-rose-700 flex items-center gap-1 font-extrabold text-xs sm:text-sm">
                    <XCircle size={14} /> Quotation Declined
                  </span>
                )}
                {quote.customerResponseAt && (
                  <span className="text-[10px] sm:text-[11px] text-[#85431E]">
              ...
              {quote.customerResponseNotes && (
                <p className="text-xs text-[#34150F] bg-white p-2.5 rounded-lg sm:rounded-xl border border-[#34150F]/10 italic">
                  "{quote.customerResponseNotes}"
                </p>
              )}
              ...
              <p className="text-[10px] sm:text-[11px] text-[#85431E]/80 pt-0.5">
                Your response is recorded. For further scope revisions, you can adjust line items during PO submission or contact support@pacifichardware.com.
              </p>
            </div>
          ) : quote.status === "APPROVED" ? (
            <div className="space-y-3">
              {/* Advance Payment Notice Box */}
              <div className="p-3 sm:p-4 bg-[#EACEAA]/20 rounded-xl sm:rounded-2xl border border-[#34150F]/15 flex flex-wrap items-center justify-between gap-2 text-xs">
                <div>
                  <span className="font-bold text-[#34150F] block text-[11.5px] sm:text-xs">
                    Advance Payment Required:{" "}
                    <span className="text-[#85431E] font-extrabold text-xs sm:text-sm">
                      {quote.advancePercentage !== null && quote.advancePercentage !== undefined
                        ? `${quote.advancePercentage}%`
                        : "30%"}
                    </span>
                ...
                {quote.customerEditCount !== undefined && quote.customerEditCount > 0 && (
                  <span className="text-[9px] font-bold text-[#85431E] bg-[#EACEAA]/50 px-2 py-0.5 rounded border border-[#34150F]/10">
              ...
              <div className="space-y-1">
                <label className="text-[11px] sm:text-xs font-bold text-[#34150F]">Optional Remarks / PO Number</label>
                <textarea
                  rows={2}
                  value={decisionNotes}
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-1">
                <button
                  ...
                  className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-2.5 sm:py-3.5 px-4 sm:px-6 rounded-lg sm:rounded-xl transition-all shadow-2xs flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs py-2.5 sm:py-3.5 px-4 sm:px-6 rounded-lg sm:rounded-xl transition-all shadow-2xs flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
                  <XCircle size={14} />
        ...
        {quote.revisions && quote.revisions.length > 0 && (
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-[#34150F]/10 shadow-md space-y-3 sm:space-y-4 print:hidden">
            <h3 className="text-sm sm:text-base font-bold text-[#34150F] flex items-center gap-1.5">
              <History size={15} className="text-[#D39858]" />
            ...
            <div className="space-y-2.5">
              {quote.revisions.map((rev) => (
                <div
                  key={rev.id}
                  className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-[#EACEAA]/15 border border-[#34150F]/10 text-xs space-y-1.5"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-[#34150F] text-[11px] sm:text-xs">
                      {rev.changedBy === "CUSTOMER" ? "Client Proposed Revision" : "Admin Approved Terms"}
                    </span>
                    <span className="text-[10px] sm:text-[11px] text-[#85431E]">
                  ...
                  {rev.remark && (
                    <p className="text-xs text-[#34150F] bg-white p-2 rounded-lg sm:rounded-xl border border-[#34150F]/10 italic">
                      "{rev.remark}"
                    </p>
                  )}

                  <div className="flex flex-wrap items-center gap-3 text-[10px] sm:text-[11px] text-[#85431E]">
