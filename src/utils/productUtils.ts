...
export function normalizeRawProduct(item: any): Product {
  ...
  return {
    ...
    material: item.material?.name || (typeof item.material === 'string' ? item.material : (item.specifications?.material || item.finish || 'Solid Brass / Stainless Steel')),
    materialId: item.materialId || item.material_id || null,
    materialObj: item.materialObj || (typeof item.material === 'object' ? item.material : null),
    frequentlyPairedIds: Array.isArray(item.frequentlyPairedIds) ? item.frequentlyPairedIds : Array.isArray(item.frequently_paired_ids) ? item.frequently_paired_ids : [],
    frequentlyPairedProducts: Array.isArray(item.frequentlyPairedProducts) ? item.frequentlyPairedProducts.map(normalizeRawProduct) : [],
